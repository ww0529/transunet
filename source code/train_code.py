...
from data_preparation import V4Dataset, create_dataloaders
...
class Sinusoidal3DPositionEncoding(nn.Module):
    ...
    def __init__(self, channels, max_len=64):
        ...
        # Divide channels equally among 3 dimensions
        # If channels is not divisible by 3, distribute remainder
        base_dim = channels // 3
        remainder = channels % 3
        dims = [base_dim + (1 if i < remainder else 0) for i in range(3)]
        d_z, d_y, d_x = dims

        # Create position indices
        pos_z = torch.arange(max_len).float()
        pos_y = torch.arange(max_len).float()
        pos_x = torch.arange(max_len).float()

        # Compute div_term for each dimension
        div_term_z = 10000 ** (2 * torch.arange(d_z).float() / max(d_z, 1))
        div_term_y = 10000 ** (2 * torch.arange(d_y).float() / max(d_y, 1))
        div_term_x = 10000 ** (2 * torch.arange(d_x).float() / max(d_x, 1))

        # Compute sin/cos encodings for each dimension
        z_sin = torch.sin(pos_z.unsqueeze(1) / div_term_z)  # [max_len, d_z]
        z_cos = torch.cos(pos_z.unsqueeze(1) / div_term_z)  # [max_len, d_z]
        z_enc = torch.cat([z_sin, z_cos], dim=1)[:, :d_z]  # [max_len, d_z]

        y_sin = torch.sin(pos_y.unsqueeze(1) / div_term_y)  # [max_len, d_y]
        y_cos = torch.cos(pos_y.unsqueeze(1) / div_term_y)  # [max_len, d_y]
        y_enc = torch.cat([y_sin, y_cos], dim=1)[:, :d_y]  # [max_len, d_y]

        x_sin = torch.sin(pos_x.unsqueeze(1) / div_term_x)  # [max_len, d_x]
        x_cos = torch.cos(pos_x.unsqueeze(1) / div_term_x)  # [max_len, d_x]
        x_enc = torch.cat([x_sin, x_cos], dim=1)[:, :d_x]  # [max_len, d_x]

        # Create 3D positional encoding grid
        pe = torch.zeros(max_len, max_len, max_len, channels)

        # Assign encodings: each position (z, y, x) gets z_enc[z] + y_enc[y] + x_enc[x]
        for z in range(max_len):
            for y in range(max_len):
                for x in range(max_len):
                    pe[z, y, x, :d_z] = z_enc[z]
                    pe[z, y, x, d_z:d_z+d_y] = y_enc[y]
                    pe[z, y, x, d_z+d_y:] = x_enc[x]
...
class PhysicsInformedUNet(nn.Module):
    ...
    def forward(self, x):
        # x shape: [B, C, D, H, W]
        # C=3 (gz, gzz, depth_index) for joint mode, or C=2 (gz, depth_index) for gz mode
        # Now we use the full 3D input instead of just the first depth layer

        B, C, D, H, W = x.shape

        # Process each depth layer through 2D encoder to extract surface features
        # Reshape to [B*D, C, H, W] for batch processing
        x_2d_input = x.permute(0, 2, 1, 3, 4).reshape(B * D, C, H, W)

        # 2D encoder processes all depth layers
        feat_2d, encoder_features = self.encoder_2d(x_2d_input)  # [B*D, C_enc, H', W']

        # Reshape back to [B, D, C_enc, H', W']
        C_enc = feat_2d.shape[1]
        H_enc = feat_2d.shape[2]
        W_enc = feat_2d.shape[3]
        feat_2d = feat_2d.reshape(B, D, C_enc, H_enc, W_enc)

        # Aggregate depth information: average pooling across depth
        feat_2d_agg = feat_2d.mean(dim=1)  # [B, C_enc, H', W']

        # Lift 2D features to 3D
        feat_3d = self.lifting(feat_2d_agg)  # [B, C_lift, D, H', W']

        # Apply transformer for 3D context
        feat_3d = self.transformer(feat_3d)  # [B, C_lift, D, H', W']

        # Fuse with 2D surface features
        # CrossDimAttention expects 4D feat_2d [B, C, H, W] and will expand it internally
        feat_3d = self.cross_dim_attention(feat_3d, feat_2d_agg)  # [B, C_lift, D, H', W']

        # Decode to density prediction
        out = self.decoder_3d(feat_3d)  # [B, 1, D, H, W]
...
class Morphology3D(nn.Module):
    """3D Morphology Operations (Diff approximation)"""
    def __init__(self, kernel_size=3):
        super().__init__()
        self.pool = nn.MaxPool3d(kernel_size, stride=1, padding=kernel_size//2)
        self.neg_pool = nn.MaxPool3d(kernel_size, stride=1, padding=kernel_size//2)
...
class ComprehensiveLoss(nn.Module):
    """Comprehensive loss function with optional dynamic weight balancing.

    Combines multiple loss terms (depth, focus, gradient, physics, edge, morphology, boundary, contrast)
    with configurable weights. Supports dynamic weight adjustment via AdaptiveLossBalancer to maintain
    consistent loss scales throughout training.

    Dynamic weights are applied as multipliers to each loss component:
    - Early epochs: Scales rapidly adjust to balance loss magnitudes
    - Later epochs: Scales stabilize, providing consistent balancing
    - All epochs: Maintains loss components at similar orders of magnitude
    """
    ...
    def __init__(self, config: V4Config, forward_op: DifferentiableForward):
        ...
        self.dynamic_weights = None  # Will be set by loss balancer

    def set_dynamic_weights(self, weights_dict: Dict[str, float]):
        """Set dynamic loss weights from AdaptiveLossBalancer.

        These weights are applied as multipliers to each loss component to maintain
        consistent scales throughout training. Set to None to disable dynamic weighting.
        """
        self.dynamic_weights = weights_dict
    ...
    def forward(self, pred, target, obs_gravity):
        ...
        # Apply dynamic weights if available (early training phase)
        if self.dynamic_weights is not None:
            for key in self.dynamic_weights:
                if key in losses:
                    losses[key] = losses[key] * self.dynamic_weights[key]
    ...
    def _gradient_loss(self, pred, target):
        ...
        return (torch.mean((dz_p - dz_t)**2) + torch.mean((dy_p - dy_t)**2) + torch.mean((dx_p - dx_t)**2))

    def _edge_loss(self, pred, target):
        def compute_edge(x):
            return torch.abs(x[:, :, 1:, :, :] - x[:, :, :-1, :, :]), torch.abs(x[:, :, :, 1:, :] - x[:, :, :, :-1, :]), torch.abs(x[:, :, :, :, 1:] - x[:, :, :, :, :-1])
        edge_z_p, edge_y_p, edge_x_p = compute_edge(pred)
        edge_z_t, edge_y_t, edge_x_t = compute_edge(target)
        weight_z, weight_y, weight_x = 1.0 + 5.0 * edge_z_t.detach(), 1.0 + 5.0 * edge_y_t.detach(), 1.0 + 5.0 * edge_x_t.detach()
        return torch.mean(weight_z * (edge_z_p - edge_z_t)**2) + torch.mean(weight_y * (edge_y_p - edge_y_t)**2) + torch.mean(weight_x * (edge_x_p - edge_x_t)**2)

    def _morphology_loss(self, pred, target):
        pred_opened, target_opened = self.morphology.opening(pred), self.morphology.opening(target)
        pred_closed, target_closed = self.morphology.closing(pred), self.morphology.closing(target)
        return F.mse_loss(pred_opened, pred.detach()) + F.mse_loss(pred_closed, pred.detach()) + 0.5 * (F.mse_loss(pred_opened, target_opened) + F.mse_loss(pred_closed, target_closed))
    ...
    def _boundary_loss(self, pred, target):
        ...
        edge_t_z, edge_t_y, edge_t_x = dz_t / (dz_t.max() + 1e-6), dy_t / (dy_t.max() + 1e-6), dx_t / (dx_t.max() + 1e-6)
        weight_z, weight_y, weight_x = 1.0 + 10.0 * edge_t_z.detach(), 1.0 + 10.0 * edge_t_y.detach(), 1.0 + 10.0 * edge_t_x.detach()
        loss_z, loss_y, loss_x = torch.mean(weight_z * (dz_p - dz_t)**2), torch.mean(weight_y * (dy_p - dy_t)**2), torch.mean(weight_x * (dx_p - dx_t)**2)
...
class AdaptiveLossBalancer:
    ...
    Maintains running averages of loss scales throughout training to keep loss components
    at consistent orders of magnitude. Uses exponential moving average with momentum.

    Early epochs: Scales are initialized and rapidly adjusted to balance loss components.
    Later epochs: Scales stabilize and provide consistent balancing across all training.
    """

    def __init__(self, loss_names, target_scale=0.5, momentum=0.95):
        self.loss_names = loss_names
        self.target_scale = target_scale  # Target scale for each loss component
        self.momentum = momentum  # EMA momentum (higher = more stable, slower adaptation)
        self.running_scales = {name: 1.0 for name in loss_names}
        self.initialized = False

    def update(self, losses: Dict[str, torch.Tensor]):
        """Update running scale factors based on current loss values.

        Uses exponential moving average to smoothly adapt scales:
        - Early updates: Rapid initialization
        - Later updates: Gradual refinement with momentum
        """
        for name, loss in losses.items():
            ...
            # Compute scale to bring this loss to target_scale
    ...
    def get_balanced_loss(self, losses: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Return the balanced total loss (for reference, not used in current implementation)."""
        total = 0
        for name, loss in losses.items():
    ...
    def get_scales(self) -> Dict[str, float]:
        """Get current scale factors for all loss components."""
...
class HighResVisualizer:
    ...
    def _plot_voxel_body(self, ax, density: np.ndarray, dx: float, dz: float,
        ...
        # Safe normalization with zero protection
        density_max = density.max()
        density_min = density.min()

        if density_max > 1e-6:
            colors[..., :3] = plt.cm.YlGn((density - density_min) / (density_max - density_min + 1e-6))[..., :3]
        else:
            colors[..., :3] = plt.cm.YlGn(0.5)[..., :3]

        # Handle negative densities safely
        if density_min < -1e-6:
            neg_mask = density < 0
            neg_normalized = (-density[neg_mask]) / (np.abs(density_min) + 1e-6)
            colors[neg_mask, :3] = plt.cm.RdPu(neg_normalized)[..., :3]
...
class V4Trainer:
    ...
    def __init__(self, config: V4Config):
        ...
        # Only initialize discriminator if adversarial training is enabled
        ...
        if config.w_adv > 0:
            ...
            print("Adversarial training disabled (w_adv=0)")
        ...
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer,
            T_max=config.epochs * len(self.train_loader),
            eta_min=1e-6
        )
        ...
        self.train_history = {
            ...
            'loss_components': {
                ...
                'physics': [], 'l1': [], 'morph': [], 'boundary': [], 'contrast': []
            }
        }
        # Only add 'adv' to loss_components if adversarial training is enabled
        if config.w_adv > 0:
            self.train_history['loss_components']['adv'] = []
    ...
    def train_epoch(self, epoch, loss_balancer=None):
        self.model.train()
        ...
        for batch_idx, (inputs, targets, obs_gravity) in enumerate(self.train_loader):
            ...
            # Adversarial training step (only if discriminator is enabled)
            if self.discriminator is not None and batch_idx % 2 == 0:
            ...
            if self.scaler:
                with autocast('cuda'):
                    ...
                    # Add adversarial loss if discriminator is enabled
                    if self.discriminator is not None:
    ...
    def train(self):
        ...
        loss_balancer = AdaptiveLossBalancer(
            ['depth', 'focus', 'gdl', 'physics', 'edge', 'morph', 'boundary', 'contrast'],
            target_scale=0.5,
            momentum=0.95
        )
        ...
        for epoch in range(1, self.config.epochs + 1):
            ...
            train_loss, loss_components = self.train_epoch(epoch, loss_balancer)

            # Update loss balancer with current epoch's loss components
            # This maintains running averages of loss scales throughout training
            loss_balancer.update(loss_components)
            scales = loss_balancer.get_scales()

            # Apply dynamic weights to criterion for all epochs
            # Early epochs: scales are being initialized
            # Later epochs: scales stabilize and provide consistent balancing
            self.criterion.set_dynamic_weights(scales)

            if epoch <= 5 or epoch % 10 == 0:
                print(f"\n[Epoch {epoch}] Loss balancing scales:")
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=V4Config.epochs)
    parser.add_argument('--batch_size', type=int, default=V4Config.batch_size)
    parser.add_argument('--lr', type=float, default=V4Config.lr)
