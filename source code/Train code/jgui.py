...
V4 重力反演专业版 GUI
======================
一个健壮的、功能完整的 GUI 用于测试 V4 重力反演模型
...
try:
    # Import from local train_code
    from train_code import PhysicsInformedUNet
    from config import V4Config
    HAS_V4 = True
    print("[OK] V4 model imported from train_code successfully")
except ImportError as e:
    print(f"[ERROR] Failed to import V4 model: {e}")
    import traceback
    traceback.print_exc()
...
class DataManager:
    ...
    def load_vti(self, filepath: str) -> bool:
        ...
        try:
            ...
            if nz > 1:
                ...
                # 使用正演计算重力作为模型输入
                try:
                    from transunet_v4 import FastGravityForward
    ...
    def get_input_tensor(self, nz: int = 16) -> Optional[torch.Tensor]:
        """构建 V4 模型输入张量 [1, 3, D, H, W]"""
        ...
        # 检查是否有VTI真实密度数据
        if hasattr(self, 'density_3d') and self.density_3d is not None:
            # VTI模式：使用 FastGravityForward 精确计算 gz 和 gzz（与 gui.py 相同）
            try:
                from transunet_v4 import FastGravityForward
                m_true = self.density_3d
                tnz, tny, tnx = m_true.shape
                
                # 直接使用原始密度进行正演（与 gui.py 完全一致）
                fwd = FastGravityForward((tnz, tny, tnx), dx=self.dx, dz=self.dx, mode='joint')
                field_data = fwd.forward(m_true)
                gz_raw = field_data[0]
                gzz_raw = field_data[1]
                
                print(f"[VTI Mode] Using FastGravityForward: gz=[{gz_raw.min():.2f}, {gz_raw.max():.2f}], gzz=[{gzz_raw.min():.2f}, {gzz_raw.max():.2f}]")
                
                # 缩放到 32x32
                from scipy.ndimage import zoom as nd_zoom
                scale_y = ny / gz_raw.shape[0]
                scale_x = nx / gz_raw.shape[1]
                gz_32 = nd_zoom(gz_raw, [scale_y, scale_x], order=1)
                gzz_32 = nd_zoom(gzz_raw, [scale_y, scale_x], order=1)
                
                # 动态归一化（与 gui.py 一致）
                gz_max = np.abs(gz_32).max() + 1e-8
                gzz_max = np.abs(gzz_32).max() + 1e-8
                gz_norm = gz_32 / gz_max
                gzz_norm = gzz_32 / gzz_max
                print(f"[VTI Mode] Normalization: gz_max={gz_max:.2f}, gzz_max={gzz_max:.2f}")
                
            except Exception as e:
                print(f"[VTI Mode] FastGravityForward failed: {e}, fallback to filter")
                gzz_max = np.abs(self.gzz_32).max() + 1e-8
                gzz_norm = self.gzz_32 / gzz_max
                gz_approx = uniform_filter(self.gzz_32, size=5)
                gz_max = np.abs(gz_approx).max() + 1e-8
                gz_norm = gz_approx / gz_max
        else:
            # 普通模式：只有 Gzz 数据，使用均值滤波近似
            gzz_max = np.abs(self.gzz_32).max() + 1e-8
            gzz_norm = self.gzz_32 / gzz_max
            gz_approx = uniform_filter(self.gzz_32, size=5)
            gz_max = np.abs(gz_approx).max() + 1e-8
        ...
        return input_vol.unsqueeze(0)  # [1, 3, D, H, W]
    ...
    def _on_load_model(self):
        ...
        if path:
            if self.model_mgr.load_model(path):
                ...
                self.status.showMessage(f"模型已加载: {path}")
            else:
